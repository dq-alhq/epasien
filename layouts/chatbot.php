...
$query_chatbot_poli = bukaquery("SELECT DISTINCT poliklinik.kd_poli, poliklinik.nm_poli 
                                FROM jadwal 
...
$query_chatbot_jadwal = bukaquery("SELECT 
        dokter.kd_dokter,
        dokter.nm_dokter,
        poliklinik.kd_poli,
        poliklinik.nm_poli,
...
<div id="epasien-chatbot-container" class="fixed bottom-6 right-6 z-100 flex flex-col items-end gap-3 font-sans">
    ...
    <div id="chatbot-tip-badge" class="bg-white border border-slate-200/80 px-4 py-2.5 rounded-2xl shadow-lg flex items-center gap-2 max-w-60 text-xs font-semibold text-slate-700 animate-bounce transition-all duration-500 mr-2">
    ...
    <!-- Trigger Button -->
    <button id="chatbot-trigger" onclick="toggleChatbot()" class="relative flex h-14 w-14 items-center justify-center rounded-full bg-linear-to-tr from-nu-600 to-emerald-500 text-white shadow-xl shadow-emerald-700/20 hover:scale-105 border border-white/20 transition-all duration-300 cursor-pointer">
        <!-- Pulse effect rings -->
    ...
    <!-- Chat Window Box -->
    <div id="chatbot-window" class="fixed bottom-24 right-6 z-100 w-96 max-w-[calc(100vw-32px)] h-137.5 max-h-[calc(100vh-120px)] flex flex-col rounded-[28px] bg-white/95 backdrop-blur-md border border-slate-200/80 shadow-2xl overflow-hidden transform scale-0 opacity-0 origin-bottom-right transition-all duration-300 pointer-events-none">

        <!-- Header -->
        <div class="bg-linear-to-r from-nu-700 to-nu-600 text-white px-5 py-4 flex items-center justify-between shadow-sm">
...
<script>
    ...
    function showMenuOptions() {
        ...
        const options = [{
                text: 'Ada poli spesialis apa saja?',
                action: 'ask_polis'
            },
            {
                text: 'Info jadwal poli (...nama poli)',
                action: 'ask_jadwal_pilihan'
            },
            {
                text: 'Gimana cara daftar poliklinik?',
                action: 'ask_daftar'
            },
            {
                text: 'Dimana alamat rumah sakit?',
                action: 'ask_alamat'
            }
        ];
